...
const DetailOrder = () => {
  ...
  const orderDetail = ordersData.find(order => order.kodeOrder === kodeOrder);
            ...
            {orderDetail ? (
              <div className="py-4 flex justify-between items-center">
                ...
                <div className="flex gap-16">
                  <div className="bg-gray p-2 min-w-36 leading-0">
                    <p className="font-bold">Kode Order:</p>
                    <p>{orderDetail.kodeOrder}</p>
                  </div>
                  <div className="bg-gray p-2 min-w-36">
                    <p className="font-bold">Pelanggan:</p>
                    <p>{orderDetail.pelanggan}</p>
                  </div>
                  <div className="bg-gray p-2 min-w-36">
                    <p className="font-bold">Meja:</p>
                    <p>{orderDetail.meja}</p>
                  </div>
                </div>

                {/* Tombol Tambah */}
                <button className="flex items-center gap-2 bg-kuning text-primary px-4 py-2 rounded">
                ...
                <tbody>
                  {orderDetail.items.map((item, index) => (
                    ...
                    >
                      ...
                      <button className="bg-[#CFD43E] text-white px-3 py-1 rounded flex items-center gap-1" onClick={() => navigate(`/orders/${order.kodeOrder}`)}>
