...
const FeaturedMenu = () => {
  ...
  const items = menus.map((menu) => (
    <div key={menu.id} className="bg-secondary shadow-md rounded-lg p-4 min-w-[250px] md:w-auto relative mx-2 ">
      {/* Gambar Menu */}
      <div className="relative">
        <img src={menu.image} alt={menu.name} className="w-full h-48 object-cover rounded-t-lg" />
        {/* Tombol Like */}
        <button onClick={handleLike} className="absolute top-2 left-2 bg-white p-2 rounded-full shadow-md">
      ...
      {/* Deskripsi Singkat */}
      <p className="text-sm text-gray-600 mt-2">{menu.description.length > 35 ? `${menu.description.substring(0, 35)}...` : menu.description}</p>
      {/* Baca Selengkapnya */}
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <button className="px-4 py-2 bg-kuning text-primary rounded-lg hover:bg-primary-dark transition">Beli Langsung</button>
      </div>
    </div>
  ));
  ...
  return (
    <div className="bg-primary p-6">
      <h2 className="text-5xl font-bright text-center text-kuning my-6">Menu Unggulan</h2>
      <AliceCarousel
        mouseTracking
        items={items}
        responsive={{
